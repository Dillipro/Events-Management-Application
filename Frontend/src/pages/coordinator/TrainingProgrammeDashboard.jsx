...
const [formData, setFormData] = useState(initialFormState);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://localhost:5000/api/coordinator/programmes');
...
const handleEdit = async (id) => {
  try {
    ...
    const response = await axios.get(`http://localhost:5000/api/coordinator/programmes/${id}`, {
...
const handleDelete = async (id) => {
  ...
  try {
    const token = localStorage.getItem('token');
    await axios.delete(`http://localhost:5000/api/coordinator/programmes/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
...
const handleNext = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (editId) {
  // Edit mode
  await axios.put(`http://localhost:5000/api/coordinator/programmes/${editId}`, formPayload, {
    headers: {
  ...
  // Create mode
  await axios.post('http://localhost:5000/api/coordinator/programmes', formPayload, {
    headers: {
      'Content-Type': 'multipart/form-data',
      'Authorization': `Bearer ${token}`
    }
  ...
  enqueueSnackbar('Note Order created successfully!', { variant: 'success' });
    ...
    // 5. Send API request
    const response = await axios.post('http://localhost:5000/api/coordinator/programmes', formPayload, {
      headers: {
            ...
            <Card sx={{ 
              ...
              <CardActions sx={{ justifyContent: 'flex-end', p: 2 }}>
                <Button 
                  size="small" 
                  startIcon={<Receipt />}
                  onClick={() => window.open(`http://localhost:5000/api/coordinator/programmes/${event._id}/pdf`, '_blank')}
                >
                  PDF
                </Button>
                <Button 
